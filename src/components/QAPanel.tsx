...
function QAItems() {
  const [itemStates, setItemStates] = useState<Record<string, ItemState>>(() =>
    Object.fromEntries(
      DEMO_QA_ITEMS.map((q) => [q.id, { answer: "", phase: "idle" }]),
    ),
  );
        ...
        return (
          <div key={qa.id} className="p-5 space-y-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <span
                className={`text-xs font-semibold px-2.5 py-1 rounded-full border ${TAG_STYLES[qa.tagType]}`}
              >
                {qa.tag}
              </span>
            ...
            {state.phase === "idle" && (
              <button
                type="button"
                onClick={() => setPhase(qa.id, "answering")}
